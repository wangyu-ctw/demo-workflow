...
import { OutputPanelController } from "./outputPanel/controller";
...
export const createGraphSession = (canvas: HTMLCanvasElement, options: GraphSessionOptions = {}) => {
  ...
  const outputPanelController = new OutputPanelController(canvas);
  ...
  outputPanelController.setNodes(nodeInstances);
  ...
  canvasView.drawCustom = (ctx) => {
    ...
    outputPanelController.setTransform(scale, offset, pixelRatio);
    outputPanelController.setInteractionActive(canvasView.isInteractionActive());
    outputPanelController.render(ctx);
  };
